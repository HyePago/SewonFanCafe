...
<html lang="en">
    ...
    <body>
    	<%
    		...
    		String select = request.getParameter("find_select");
        ...
        <form action="notice/insert.html" id="myForm" class= "myForm" name="myForm" method="post">
            <table width="700px" class="notice_out_table">
                <tbody>
                    <tr>
                        <td> <div class="notice_logo">통합 공지</div></td>
                    </tr>
                            ...
                            <table class="notice_inner_table">
                                <tbody>
                                    ...
                                    <%
                                    	try {
                                    		if(select.equals("1")) {
                                    			SQL = "SELECT * FROM notice WHERE title like '%" + text + "%' ORDER BY timestamp DESC LIMIT 10 OFFSET " + ((current_page - 1) * 10);
                                    		} else if(select.equals("2")) {
                                    			SQL = "SELECT * FROM notice WHERE content like '%" + text + "%' ORDER BY timestamp DESC LIMIT 10 OFFSET " + ((current_page - 1) * 10);
                                    		}
                                    		pstmt = conn.prepareStatement(SQL);
        ...
        <script>
			function find_click() {
				document.myForm.action = "notice_find.jsp?page=1";
